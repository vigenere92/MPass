...
MPass is distributed in the hope that it will be useful,
...
class Engine
{
    ...
    public:
        ...
        void is_satisfiable(string query)
        {
            ...
            query += "(check-sat)";
            query += "(get-model)";
            string input_path = homedir+(string)"/MPass_result/smt_solver_input_presburger_formula.txt";
            ofstream myfile;
            ...
            ofstream temp_file;
            string output_path = homedir+(string)"/MPass_result/smt_solver_output.txt";
        ...
        void filter_data(string type)
        {
            if(type == "new")
            {
                ...
                struct passwd *pw = getpwuid(getuid());
                const char *homedir = pw->pw_dir;
                string tex_path =  homedir+(string)"/MPass_result/Result/data.tex";
        ...
        void output_tex(Constraint Presburger_formula)
        {
            ...
            string result_path = homedir + (string)"/MPass_result/Result";
...
int main(int argv, char *argc[])
{
    ...
    else if (argv==1)
    {
        ...
        cin >> settings_file;
        Engine MPass(settings_file, "false");
    }
    else
    {
        cout << "Wrong argument passed during execution !" << endl;
